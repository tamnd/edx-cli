// Package cli assembles the edx command tree from the edx
...
// NewApp assembles the kit application from the edx domain. The
...
//
// To add a command, declare it in edx/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := edx.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(edx.Domain{}).Register(app)
