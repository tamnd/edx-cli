...
describes what users get. Pick a format with `-o`, or let edx choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
edx <command> -o table   # aligned columns for reading
edx <command> -o jsonl   # one JSON object per line, for piping
edx <command> -o json    # a single JSON array
edx <command> -o csv     # spreadsheet friendly
edx <command> -o tsv     # tab-separated
edx <command> -o url     # just the URL column
edx <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
edx <command> --fields id,title,url
```
...
```bash
edx <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
edx <command>            # a table, because this is a terminal
edx <command> | wc -l    # JSONL, because this is a pipe
```
